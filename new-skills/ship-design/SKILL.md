---
name: ship-design
description: "新 ShipKit Design 阶段。基于 requirements.md 和 spec 生成合一技术设计：API Contract、数据模型、前后端方案，并在 Build 前等待用户确认。"
---
...
基于 `requirements.md(status: ready)` 产出可实现的 `design.md`。一份文档同时包含 API Contract、数据模型、前端设计、后端设计和 AC 覆盖映射。

## 输入

- `requirements.md`，必须 `status: ready`。
- `meta.yml.scenario`、`spec_refs`、可选 `requested_design_template`。
- `ship-spec` 加载的 API 规范、前端模式、数据模型规范、技术栈。
- Design Reference Template：项目级模板或 `new-skills/ship-orchestrator/templates/design-reference/` 内置模板。
- 现有代码结构（只读探索）。
## 流程

1. 运行 `validate_requirements.py`；失败则退回 Understand。
...
6. 按场景调用 `ship-grill-me` 做设计审查；有 blocking 就修设计或提问。
7. 运行 `validate_design.py`；有 error 不得 ready。
...
| `quick_start` | 默认跳过 |
| `full_flow` | 必须触发 |
| `prd_direct` | 有技术风险、规范冲突、错误处理缺失时触发 |
...
4. 必须写入 `meta.yml.design_template_reason`，说明选择依据。
5. `design.md` frontmatter 不重复模板机器事实；正文 `## 方案模板引用` 必须与 `meta.yml.design_template_ref` 一致。
6. `full_flow`、`prd_direct`、`split_first` 子 feature 必须显式记录模板引用；`quick_start` 可降级为“未使用”，但要说明原因。
7. 多模板命中时只选一个主模板，其他最多 2 个作为候选/辅助 checklist，不把模板拼成怪物文档。
8. 项目 spec 优先级高于模板；冲突时以 `requirements/AC > 项目 spec > 现有代码事实 > 参考模板 > AI 默认习惯` 为准。
...
只有用户明确确认 `yes/开始/确认实现` 后，`ship-orchestrator` 才能把 `current_stage` 改为 `build`。

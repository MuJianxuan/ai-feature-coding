...
- **元数据集中化**: feature 级 `meta.yml` 一次维护，砍掉 6 个文档的 frontmatter 重复维护
...
| 13 测试 | `ship-verify` | 阶段 | 测试代码 + 报告 |
| 14 验收 | `ship-handoff` | 阶段 | verification.md + handoff.md |
...
orchestrator 读 meta.yml，找到 `current_stage` 和 DOING 任务（如有），从断点恢复。
...
| 元数据维护 | 6 处 frontmatter 重复 | meta.yml 集中一次 |
...
| 反模式 | 正确做法 |
|--------|---------|
| 简单 bug fix 走完整流程 | orchestrator 识别为简单任务直接编码 |

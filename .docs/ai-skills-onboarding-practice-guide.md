...
>
> 核心目标：让使用者明确知道**什么时候可以启动这套 workflow、启动后每一步如何推进、如何避免普通任务误触发、如何验证配置没有漂移**。

## 1. 一句话理解这套 workflow

这是一套 **explicit opt-in / orchestrator-routed-only** 的 AI feature delivery workflow。

也就是说：

- 普通“帮我写代码 / 查 bug / 设计方案 / 拆任务 / 做验证”不会自动触发这套 workflow。
...
| `skills/ai-feature-orchestrator/scripts/validate_ai_skills.py` | 本 workflow 的 smoke test。 |

## 3. 角色与阶段

| 阶段 | Skill | 主要产物 | 进入条件 |
| --- | --- | --- | --- |
| 总调度 | `ai-feature-orchestrator` | 阶段判断、route payload | 用户显式要求启动 workflow。 |
...
| 任务拆解 | `ai-task-planning` | `tasks.md` | design 已 ready，可拆成原子任务。 |
...
除非用户明确写出 skill 名或“使用这套 AI feature workflow / skills workflow”。
...
- 如果可拆任务，`stage_status: ready`。

停止点：输出下一阶段建议，不自动进入 task planning。

### 8.4 Task Planning

目标：把设计拆成原子、可验证、按依赖排序的任务。
...
- 读取 requirements / investigation / design / tasks。
- 检查工作区状态。
- 不覆盖用户未提交改动。
- 将当前任务标记为 `DOING`。
...
当 workflow 中断或 `tasks.md` 存在 `DOING`：
...
5. 运行 smoke test。
6. 用只读方式模拟一个已有 feature 目录的阶段判断。
7. 真正执行时，一次只推进一个阶段，并在阶段结束后等待用户确认。

# Coding Skills Onboarding 实践指南

> 适用对象：第一次使用、维护或接入本仓库 `skills/coding-*` 工作流的人。
>
> 核心目标：让使用者明确知道**什么时候可以启动 Coding Feature Workflow、启动后每一步如何推进、如何避免普通任务误触发、如何验证配置没有漂移**。

## 1. 一句话理解 Coding Feature Workflow

这是一套 **explicit opt-in / orchestrator-routed-only** 的 **Coding Feature Workflow**。

也就是说：

- 普通“帮我写代码 / 查 bug / 设计方案 / 拆任务 / 做验证”不会自动触发 Coding Feature Workflow。
- 只有用户明确指定 `coding-feature-orchestrator` 或某个阶段 skill，才允许启动。
- 阶段 skill 被动触发时，必须由 `coding-feature-orchestrator` 显式路由，并携带完整 route payload。
- 默认一次只推进一个阶段；跨阶段继续需要用户再次确认。
- 本指南是 onboarding 辅助说明；如与 `skills/coding-feature-orchestrator/WORKFLOW_CONTRACT.md` 冲突，以 contract 为准。
...
| 文件 | 用途 |
| --- | --- |
| `skills/coding-feature-orchestrator/WORKFLOW_CONTRACT.md` | 全局契约：activation、route、metadata、gate、safety、resume、smoke-test 规则。 |
| `skills/coding-feature-orchestrator/SKILL.md` | 总调度入口：入口模式、阶段推断、阶段路由、停止点。 |
...
| `skills/coding-feature-orchestrator/assets/feature-template/` | 新 feature 目录模板。 |
| `skills/coding-feature-orchestrator/references/golden-examples.md` | Happy path、blocked、resume、verification failed 的行为样例。 |
| `skills/coding-feature-orchestrator/scripts/validate_coding_skills.py` | Coding Feature Workflow 的 smoke test。 |

## 3. 角色与阶段

| 阶段 | Skill | 主要产物 | 进入条件 |
| --- | --- | --- | --- |
| 总调度 | `coding-feature-orchestrator` | 阶段判断、route payload | 用户显式要求启动 Coding Feature Workflow。 |
| 需求澄清 | `coding-requirement-intake` | `requirements.md` | 需求 scope / acceptance criteria 未稳定。 |
| 仓库勘察 | `coding-repo-investigation` | `investigation.md` | 需要真实代码链路、数据来源、接口证据。 |
| 技术设计 | `coding-technical-design` | `design.md` | requirements 和 investigation 已 ready。 |
| 设计审批 | orchestrator / user gate | `design.md approval_status` | design ready 后等待用户明确批准。 |
| 任务拆解 | `coding-task-planning` | `tasks.md` | design 已 ready 且 `approval_status: approved`。 |
| 编码执行 | `coding-implementation-execution` | 代码改动、任务交付记录 | tasks 中存在真实 `TODO` / `DOING` 任务。 |
| 验证收口 | `coding-verification-closeout` | `verification.md`、`handoff.md` | in-scope tasks 已完成或有明确 blocked 说明。 |
...
```text
使用 coding-feature-orchestrator，为这个需求启动一条新的 Coding Feature Workflow：<需求描述>
```
...
3. 写入初始需求资料索引。
4. 路由到 `coding-requirement-intake`。
...
下面这些请求**不应触发**Coding Feature Workflow：

| 用户请求 | 应采取的行为 |
| --- | --- |
| “帮我查一下这个 bug” | 走普通 debugging，不启动 `coding-repo-investigation`。 |
| “帮我实现这个功能” | 走普通开发流程，不启动 `coding-implementation-execution`。 |
| “写个技术方案” | 走普通方案写作，不启动 `coding-technical-design`。 |
| “拆一下任务” | 走普通任务拆解，不启动 `coding-task-planning`。 |
| “跑一下测试并总结” | 走普通验证，不启动 `coding-verification-closeout`。 |

除非用户明确写出 skill 名或“使用这套 Coding Feature Workflow / 技能工作流”。
...
当 Coding Feature Workflow 中断或 `tasks.md` 存在 `DOING`：
...
每次修改 Coding Feature Workflow 后运行：
...
- Contract 和各阶段输出规则都覆盖 `updated_at` / `evidence_complete`；`coding-task-planning` 覆盖 `task_count`。
...
- `inspect_feature_state.py` 会拒绝把 `skills/coding-feature-orchestrator/assets/feature-template/` 当成真实 feature 目录。
- Route source 已收紧为仅允许 `coding-feature-orchestrator` routed invocation，不能残留 “another legally activated skill” 这类泛化表述。
- `coding-implementation-execution` 必须优先恢复真实 `DOING` 任务，再选择用户指定任务或第一个真实 `TODO`。
- `coding-verification-closeout` 必须读取 `investigation.md`，用真实调用链和数据来源校准验证范围。
...
2. 阅读 `coding-feature-orchestrator/SKILL.md`，理解入口模式和阶段推断。
3. 阅读一个子 skill，例如 `coding-task-planning/SKILL.md`，确认 direct / routed 两种启动模式。
...
| 反模式 | 后果 | 正确做法 |
| --- | --- | --- |
| 普通 bug 排查自动触发 `coding-repo-investigation` | 缺少 feature dir，文档链路混乱。 | 只有显式指定 skill 才触发。 |

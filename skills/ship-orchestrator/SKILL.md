...
- 默认对外展示 `macro_stage`，`macro_stage.next_user_decision` 是默认用户视图字段；只在高级模式和诊断模式下展开 `current_stage`
- 每次 `current_stage` 变化都必须同步 `macro_stage.current`、`macro_stage.label`、`macro_stage.summary`、`macro_stage.next_user_decision`
...
- 不得写业务代码，除非同时满足：
  ...
  - `implementation_preflight.py --files <paths...>` 通过
...
- 若用户要求跳过 ShipKit 直接编码，必须先让用户明确说“退出 ShipKit / stop ShipKit workflow”，agent 复述后果后等待二次确认；确认后写入 `confirmation_log.type: shipkit_exit`，否则在 ShipKit 内不得开后门。
...
- 场景 E 使用 `scenario: technical_plan_provided`，必须记录已有技术方案来源、selected scope、`technical_plan_source.ignored_source_policy: out_of_scope`、`technical_plan_source.repository_scan_required: true`
...
- 场景 E 的 selected scope 确认只是启动/范围确认，不是 build 授权
...
| `backend_only` | 明确说"纯后端""只做 API""不涉及前端" | `ship-shape`, `ship-frontend-design` |
| `frontend_only` | 明确说"纯前端""只做 UI""不涉及后端" | `ship-backend-design` |

**关键规则**：
- 信号歧义时默认 `fullstack`，必须显式询问用户是否要落成单侧 scope
- `backend_only` 跳过 UI shaping 与 frontend design；不跳过 `ship-contract`、`ship-backend-design`、`ship-design-review`、`ship-delivery-plan`、`ship-plan-review`
- B/D 场景的 UIUX Material Gate 只适用于 `fullstack | frontend_only` 且 feature 涉及 UI；`backend_only` 不得插入 `ship-shape`
- scope 变更只允许在 `ship-design-review` 之前；通过设计评审后 scope 冻结

## Source Code Edit Barrier

orchestrator 必须把 workflow 产物推进和业务源码修改分开处理。源码修改屏障要求：除 workspace `feature_root` 下 `feature-*` 目录内的工作流产物、`meta.yml`、`resource/` 资料和 review / plan / discovery / design 文档外，任何业务源码、测试、配置、迁移、脚本或构建文件修改都只允许在 `ship-build` 阶段发生。
...
5. `review-plan.md.confirmation_id` 能匹配 `meta.yml.confirmation_log` 的 `hard_gate_signoff` 记录
6. `python3 skills/ship-orchestrator/scripts/implementation_preflight.py <feature-dir> --project-scope <fullstack|backend_only|frontend_only> --files <workspace-relative-path...>` 通过

`stage_transition_check.py` 与 `build_task_preflight.py` 是 implementation preflight 的内部依赖，不是人工授权源码修改的替代入口。

若任一条件不满足，必须停止源码修改，报告当前阶段和缺失门禁，并路由回对应阶段。

初始化后直接编码必须被 Source Code Edit Barrier 阻塞。NEW_FEATURE 启动确认、selected scope 确认、UIUX Material Gate 授权、接口范围确认都只推进 workflow stage，不允许修改业务源码。

**UIUX Material Gate**：
- 场景 B/D 且 `project_scope = fullstack | frontend_only`、feature 涉及 UI 时，必须检查 UIUX 材料
- 若材料缺失或不可访问，保持 `stages.ship-define.status: blocked`，`block_reason` 写 `awaiting_uiux_materials` 或在 `awaiting_materials` 中明确 UIUX 缺口
- 用户明确授权生成线框时，才可插入 `ship-shape`，并记录 `activation_mode: uiux_material_gate_insert`
- `backend_only` 下不允许 UIUX Material Gate insertion
...
该 workflow 对外显示 5 个大阶段，内部维护 14 个内部阶段名。
...
- gate 检查草案等待用户批准时 frontmatter 保持 `pending`，正文可记录 `recommended_status`
- `approved` 必须由主上下文在用户明确批准后一次性写入 `review_status`、`user_sign_off`、`signed_at`

**软门禁（Soft Gate）**：
- 适用阶段：所有非硬门禁的阶段间过渡
- 通过条件：stage_status: ready 或 complete
- `soft_optional + draft` 可经用户确认强推，并记录 `skip_log`
- `soft_blocking + blocking_gaps` 非空时不可强推，不可通过 `skip_log` 绕过；只能补材料、缩 scope 或显式转场景
- 上游文档不存在仍阻断
- `ship-design-review` 入口要求 `project_scope` 对应设计文档 ready
- `ship-delivery-plan` 入口要求 review-design approved，出口要求 `project_scope` 对应 plan
- `ship-plan-review` 入口要求对应 plan `stage_status: ready`
...
- `.docs/ship/project.yml` 是 workspace boundary 的显式配置源
- project_group 下 feature 目录写入 workspace `feature_root`
- `ship-spec` 负责 workspace resolution，解析结果写入 `spec_context` 并透传给阶段 skill
...
5. 若用户要求继续直接编码，必须先执行退出 ShipKit 二次确认流程，并记录 `confirmation_log.type: shipkit_exit`
...
**Runtime Delegation Resolution**：
- 进入节点前先确定 canonical `node_id`，例如 `ship-build.read-next-task`、`ship-verify.backend-contract`
- 普通解析先读取 `node_overrides[node_id]`，再按节点类别读取 `default_mode`，最后回退 `current_context`
...
- `ask_on_assistive_node` 控制 assistive_only 节点在 override 缺失时是否先询问用户
- delegation runtime warning 必须写入 `delegation.warnings`
...
| AR-8 | "backend_only 只做接口，所以不需要 contract/design/review/plan" | backend_only 跳过 UI shaping 与 frontend design，但不裁剪上游治理 |
...
- [ ] 若进入 `ship-build`，已运行 `implementation_preflight.py --files <paths...>` 并通过

...
| ship-tech-discovery | review-requirement.md review_status: approved | tech-research.md + tech-selection.md | 两份产物均 `stage_status: ready` |
...
- 内部完整执行 12 个阶段，适用于中大型功能
- 所有硬门禁必须通过
- 所有文档产物必须完整

fast-track 模式：
- 适用于小型功能或紧急修复
- 最小路径：`ship-intake → ship-intake-review → ship-build → ship-verify → ship-handoff`
...
- `gate_check_switchable`：只用于 `ship-intake-review` / `ship-design-review` / `ship-plan-review`
...
- stage 级：`ship-intake`、`ship-intake-review`、`ship-contract`、`ship-frontend-design`、`ship-backend-design`、`ship-design-review`、`ship-delivery-plan`、`ship-plan-review`
...
| AR-2 | "需求很简单，不需要走完整流程" | 即使简单需求，也至少执行最小路径 `ship-intake → ship-intake-review → ship-build → ship-verify → ship-handoff`，评审可简化但不可省略 |

...
5. 若 INSPECT：按 target project `feature_root` 扫描所有 feature 目录，汇总状态表格

### Empty Entry Handling（未携带资料入口）

当用户只表达“开始需求 / 使用 ship-orchestrator / 继续开发”等意图，但没有指定 `feature_dir`、也没有携带资料时，必须先执行以下流程：

1. 若当前上下文已能确定 target project，则扫描该 target project `feature_root/feature-*`；否则先让用户选定 target project。
...
5. 若没有未完成 feature：直接列出四种新建模式，让用户选择。
...
- 信号歧义时默认 `fullstack`，不询问（与 scenario 不同，scope 默认安全值不阻塞）
- 用户可在启动确认时声明 scope，也可在 `ship-tech-discovery` 完成后由 orchestrator 自动推断
- 自动推断规则：若 `tech_stack.frontend` 为空且 `tech_stack.backend` 非空，自动设为 `backend_only`（反之亦然）；推断后需回写 `meta.yml.project_scope` 和 `project_scope_evidence`
- scope 变更只允许在 `ship-design-review` 之前；一旦通过设计评审，scope 冻结
- scope 为 `backend_only` 时：`stages.ship-frontend-design.status = skipped`
- scope 为 `frontend_only` 时：`stages.ship-backend-design.status = skipped`
- `ship-contract` 在所有 scope 下默认保留

NEW_FEATURE 启动确认模板：
- 简述功能名称和目标
- 标明识别到的场景（A/B/C/D）及起点阶段
...
- orchestrator：负责 project resolution，调用 `spec_runtime.py` / `feature_meta_runtime.py sync-spec`，维护 `meta.yml.spec_context`，把规范摘要透传给阶段 skill
- 阶段 skill：根据 `spec_context` 和本阶段输入实际读取规范，并把 `referenced_spec_ids`、`spec_warnings`、`spec_checked_at` 写入产物或任务证据
- `ship-handoff`：只记录 proposal，不直接写回 target project `spec_root`；真正沉淀由用户确认后执行

project resolution 规则：

- NEW_FEATURE / CONTINUE_FEATURE / `sync-spec` 都以 target project 为边界，而不是当前 cwd
- 显式配置源是 `target project/.docs/ship/project.yml`
- NEW_FEATURE 创建 feature 目录时，必须写入 target project `feature_root`
- CONTINUE_FEATURE 优先信任已有 `meta.yml.spec_context.target_project_*` 字段，不重新按 cwd 猜测
- 无法确定 target project 时阻塞；target project 已确定但 spec 缺失时只 warning
...
3. 解析 target project；显式来源是 `target project/.docs/ship/project.yml`
4. 在 target project `feature_root` 下创建目录 `feature-YYYYMMDD-<short-name>/`
5. 创建 resource/ 子目录用于存放 PRD、原型、截图等参考资料
6. 复制 `./_templates/meta/meta.yml.template` 到 feature 目录，重命名为 `meta.yml`
7. 场景 B/D：复制 `./_templates/requirements/raw-prd-inbox.md.template` 到 `requirements.md`，复制 `./_templates/resource/README.md.template` 到 `resource/README.md`
8. 填充 meta.yml 初始字段：feature_name、feature_id、created_at、current_stage
9. 初始化 `macro_stage.current`、`macro_stage.label`
10. 初始化 `macro_stage.summary` 与 `macro_stage.next_user_decision`
11. 初始化 `delegation.default_mode: current_context`、`ask_on_parallel_stage: true`、`ask_on_assistive_node: true`
12. 初始化 `spec_context.target_project_id / target_project_root / spec_root / feature_root / resolution_source`
13. 将所有阶段的 status 初始化为 pending
14. 场景 A：`current_stage: ship-discover`，不创建 raw `requirements.md` inbox，直接进入 `ship-discover`
15. 场景 B/D：将 `stages.ship-discover.status` 和 `stages.ship-shape.status` 设为 `skipped`，`current_stage: ship-define`，`stages.ship-define.status: blocked`，`block_reason: awaiting_materials`
16. 场景 D：将 `stages.ship-define.generation_mode` 设为 `prd_direct`；场景 B：设为 `interview`
17. 场景 C：若已有 feature/code 基线，`current_stage: ship-discover` 且 `discovery_mode: evolve`；若无基线，先询问，不创建目录
...
1. 优先读取已有 feature 的 `meta.yml.spec_context.target_project_*`
2. 在已解析的 target project `feature_root` 下扫描 `feature-YYYYMMDD-*` 目录

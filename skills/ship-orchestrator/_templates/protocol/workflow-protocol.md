...
2. 产出正式 `review-*.md` 草案
3. 主代理复核该草案并按需要修订
4. 主代理先给出建议结论（例如 `recommended_status`），正式 frontmatter 在等待用户批准时仍保持 `review_status: pending`
5. 若存在 Critical / Major 问题，主代理可以直接将正式 `review_status` 写为 `rejected` 或 `revision_needed` 并阻塞推进
6. 若主代理判断可通过，必须先向用户请求明确批准；只有用户明确批准后，主代理才能一次性写入 `review_status: approved`、`user_sign_off` 与 `signed_at`
...
- `single_project` 读取 `.docs/spec/INDEX.md`；`project_group` 读取 `.docs/spec/_shared/INDEX.md`，并按当前目标项目读取 `.docs/spec/<project>/INDEX.md`。
...
- project_group 下的 spec 目标项目必须来自 `.docs/ship/project.yml.projects`；`ship-build` 必须由任务 `project:` 显式提供目标项目，不从 `allowed_files` 路径反推。
...
- `single_project` 下 `.docs/spec/INDEX.md` 是人工路由入口；`project_group` 下 `.docs/spec/INDEX.md` 是顶层导航，具体入口是 `.docs/spec/_shared/INDEX.md` 和 `.docs/spec/<project>/INDEX.md`
...
- `ship-tech-discovery`：single 读根 `INDEX.md`；project_group 读 `_shared`，并按已确认目标项目读 `<project>/INDEX.md`，再按 `stage_hooks + stack_tags` 匹配
- `ship-frontend-design`：先读 `_shared`，再按 Project Reality First 确认的 UI 项目读取 `<project>/INDEX.md` 中 `frontend/shared` 候选
- `ship-backend-design`：先读 `_shared`，再按 Project Reality First 确认的 API/service 项目读取 `<project>/INDEX.md` 中 `backend/shared` 候选
- `ship-build`：project_group 必须使用任务 `project:` 选择 `<project>/INDEX.md`；按 `stage_hooks=ship-build` 且 `applies_to` 对任务文件清单做 glob 匹配；任务文件清单必须先归一化到 workspace root 相对路径；`stack_tags / domains` 为附加过滤
- `ship-handoff`：以 `meta.yml.spec_context.referenced_spec_ids` 为基础生成 proposal，跨项目模式指向 `_shared`，单项目模式指向具体 `<project>` spec root

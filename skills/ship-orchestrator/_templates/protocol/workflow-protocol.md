...
- `spec_context` 记录最近一次规范解析结果与本 feature 已引用规范摘要：
  - `target_project_id`: 目标项目稳定标识
  - `target_project_root`: target project 根路径；固定按 project-relative 记录，默认 `"."`
  - `spec_root`: target project 下的规范目录
  - `feature_root`: target project 下的 feature 运行时产物目录
...
- `ship-spec` 的运行边界是 `target project`，不是当前 agent cwd。
- `.docs/ship/project.yml` 是 target project boundary 的唯一显式配置源。
...
- `spec_context` 必须持久化以下字段：
  ...
  - `feature_root`
  - `resolution_source`
- 只有无法确定 `target project` 时才阻塞；target project 已确定但缺少 `spec_root` / `INDEX.md` / 匹配 spec 时，统一走 Warn Then Continue。

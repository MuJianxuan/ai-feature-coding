...
`ship-grill-me` 可作为 `ship-design-review.pre-signoff` 辅助质询 hook 使用。触发点是 `review-design.md` 草案完成后、用户批准前。它不替代 review checklist，不直接写 `review_status: approved`，不替用户签字。

质询重点：

- Critical / Major 是否真的全部关闭。
- 剩余 Minor 是否可接受。
- 用户是否接受上线、性能、安全、兼容性风险。
- 是否有条件批准，需要写入 `conditions`。
- 是否应回退到 contract / frontend design / backend design 修复。

输出规则：

- grill 结果只能形成 sign-off questions、risk acceptance candidates、conditions candidates。
- `review_status: approved`、`user_sign_off`、`signed_at` 必须由主上下文在用户明确批准后一次性写入。
- 若用户回答暴露 Critical / Major 问题，不得 approved；应设置 `revision_needed` 或保持 `pending` 并回退修复。
- 若用户接受 Minor 风险，必须写入正文风险接受记录；如有条件批准，写入 `conditions` candidates，等待主上下文和用户确认。
...
6. **Pre-Signoff Grill** —— 签字前风险质询、推荐决策和条件候选
7. **用户签字** —— 用户确认评审结论的签字区域
...
- [ ] 若启用 `ship-grill-me`，Pre-Signoff Grill 只记录 sign-off questions / risk acceptance candidates / conditions candidates，未直接写 `review_status: approved`

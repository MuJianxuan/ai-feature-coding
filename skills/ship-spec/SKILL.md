...
- 规范已存在且无需修改时不要重复生成
- 不要在 `ship-build` 中途停下来写规范（完成后再沉淀）
...
在以下时机触发沉淀：
- **ship-handoff** 完成后：review 过程中发现的模式；若无新增规范，也要显式记录"无新增规范"
- 用户主动要求："把这个模式沉淀为规范"
...
| ship-build | 读取 | 每个任务开始前加载匹配规范，并记录引用的 `spec_id` |
| ship-handoff | 写入 | 检查是否有可沉淀的新规则；有则写入，无则记录"无新增规范" |
...
| "规范写了也没人看" | agent 在 `ship-build` 阶段会自动加载，这是给 AI 看的 |
...
- [ ] 每条规则有正例和反例
- [ ] 规范与当前技术栈匹配
- [ ] `ship-build` 阶段的任务描述中引用了相关规范
- [ ] `ship-handoff` 已明确记录本次是否产生新增规范

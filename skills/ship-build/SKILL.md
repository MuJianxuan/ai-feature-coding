---
name: ship-build
description: "ShipKit stage. Executes reviewed delivery-plan tasks one at a time after ship-plan-review passes."
---
...
编码执行阶段负责将 `ship-delivery-plan` 产出的任务事实源逐一转化为可运行的代码。核心原则：**Contract-First Slicing** —— 先实现契约层任务，再按 `project_scope` 推进对应端实现，且一次只执行一个任务，完成后停下等用户确认。

核心目标：
- 严格按 plan 中定义的任务顺序执行，一次一任务
- 每个任务完成后必须通过验证（测试/构建/lint）
- 保持 Scope Discipline，不修改 spec 之外的文件
- 任务事实源随 `project_scope` 裁剪：fullstack 读双 plan，单侧 scope 读对应侧 plan
...
- `review-plan.md` 已通过（`review_status: approved`，且 `user_sign_off`、`signed_at` 非空）
...
本阶段根据 `project_scope` 调整任务事实源：

| project_scope | 任务事实源 | 说明 |
|------------------------------|-----------|------|
| `fullstack` | `frontend-plan.md` + `backend-plan.md` | 双侧任务顺序都必须遵守 |
| `backend_only` | `backend-plan.md` | 只读后端任务事实源 |
| `frontend_only` | `frontend-plan.md` | 只读前端任务事实源 |

适配规则：

- 入口 gate：`review-plan.md.review_status = approved`，且 `user_sign_off`、`signed_at` 非空
...
5. **不改计划语义**：不得修改 scope、AC、依赖、allowed_files 或设计/契约含义；但允许按任务生命周期更新 status、started_at、finished_at、spec_refs、evidence、notes 等执行字段。发现计划语义有问题时，停下报告，由用户决定是回到 plan 阶段还是局部调整。
...
`implementation_preflight.py` 是唯一源码修改入口；`stage_transition_check.py` 和 `build_task_preflight.py` 只作为其内部依赖。
...
- [ ] backend_only：backend-plan.md 中所有任务为 DONE，frontend-plan.md 不存在或标记为 `na`
- [ ] frontend_only：frontend-plan.md 中所有任务为 DONE，backend-plan.md 不存在或标记为 `na`
- [ ] 契约层实现与 api-contract.md 中的 contract refs 一致
- [ ] 端到端关键路径已跑通（即使是手动验证）

...
### 7.1 Fast-track gate policy

当 `pipeline_mode: fast_track` 时，gate policy 简化为：

- `brief.md stage_status: ready` 且 `evidence_complete: true` 即可进入 implementation。
- 不要求 `design.md` 存在，不要求 design approval。
- 不要求 `tasks.md` 存在；`implementation.md` 直接记录改动。
- `implementation.md stage_status: ready` 且 `evidence_complete: true` 即可进入 verification。
- Verification 仍必须映射 `brief.md` 中的 acceptance criteria。
- Code review 为可选（用户可跳过）。

Fast-track 升级为 standard：

- 如果执行中发现 complexity 超出 `patch`（需要多文件改动、涉及 API 变更、需要 design 决策），必须停止并建议升级。
- 升级时，`brief.md` 内容拆分到 `discovery.md` + `requirements.md`，保留已有证据。
- 升级后按 standard gate policy 继续。
...
## 15. Optional collaborator contract

`agent-product-manager` 和 `agent-solution-architect` 是独立 skill，不属于 Coding Feature Workflow 的阶段 skill，不受本 contract 的 activation contract、route contract 或 upstream metadata gate 约束。

### 定位

- 它们可被用户随时单独调用，无需进入 Coding Feature Workflow。
- 在 Coding Feature Workflow 执行过程中，orchestrator 可按场景建议引入它们作为协作者，但不是强制环节。

### 适用场景

| 协作者 | 建议引入的场景 |
| --- | --- |
| `agent-product-manager` | discovery 阶段原始需求缺乏产品结构化思考；requirements 阶段 PRD 质量评审或优先级排序；用户主动要求产品策略咨询 |
| `agent-solution-architect` | discovery 阶段 `empty_project` 技术栈选型或架构方向探索；design 阶段复杂系统架构设计或多方案评审；用户主动要求架构评审或技术选型 |

### 协作边界

- 协作者不改变阶段推断、gate policy、停止点或 document metadata contract。
- 协作者的产出是当前阶段 skill 的输入参考，不替代阶段文档的写入责任。
- 当前阶段 skill 应将协作者产出的关键结论吸收到对应阶段文档中，并在证据中标注来源（例如"来源：agent-solution-architect 架构评审"）。
- orchestrator 只建议引入协作者，不强制；用户未采纳时正常继续当前阶段。
- 协作者不得直接修改 feature 目录下的阶段文档；所有阶段文档写入仍由对应阶段 skill 负责。

## 16. Metrics contract

每个 feature 目录下维护 `metrics.json`，记录执行过程中的关键事件。Metrics 是辅助文档，不参与阶段推断，写入失败不阻塞主流程。

### 事件类型

| 事件类型 | 触发时机 | 必填字段 |
| --- | --- | --- |
| `stage_enter` | 阶段 skill 开始执行时 | `stage`, `timestamp`, `trigger`（`direct_explicit` / `routed_invocation`） |
| `stage_complete` | 阶段文档标记为 `ready` / `complete` 时 | `stage`, `timestamp`, `duration_minutes`, `user_interactions` |
| `stage_blocked` | 阶段文档标记为 `blocked` 时 | `stage`, `timestamp`, `blocker` |
| `blocker_resolved` | 阻塞解除时 | `stage`, `timestamp`, `original_blocker`, `resolution` |
| `composition_call` | 组合调用发生时 | `caller`, `callee`, `call_type`（`advisory` / `utility` / `validation`）, `timestamp` |
| `error_recovery` | 错误恢复流程触发时 | `stage`, `task_id`, `error_type`, `recovery_action`, `recovery_result`, `timestamp` |
| `pipeline_upgrade` | fast-track 升级为 standard 时 | `timestamp`, `reason` |
...
1. 每个阶段 skill 在进入和完成时必须追加对应事件到 `metrics.json`。
2. `duration_minutes` 由 `stage_complete` 事件的 `timestamp` 减去对应 `stage_enter` 事件的 `timestamp` 计算。
3. `user_interactions` 统计该阶段内用户主动输入的次数（不含系统自动触发）。
4. `summary` 在每次事件追加后同步更新。
5. Feature 完成（`handoff.md stage_status: complete`）时，`summary.total_duration_hours` 由 `created_at` 到最后一个事件的 `timestamp` 计算。
6. `metrics.json` 不参与 upstream metadata gate、阶段推断或 smoke-test 的阶段文档校验。
7. 如果 `metrics.json` 不存在或格式损坏，阶段 skill 应尝试重建空结构后继续，不得因此停止主流程。

## 17. Error recovery contract

本 section 定义 `coding-implementation-execution` 阶段的错误恢复机制。其他阶段遇到阻塞时按 `stage_status: blocked` 处理，不适用本 contract。

### Checkpoint 机制

每个任务在以下时机自动创建 checkpoint，记录在 `tasks.md` 当前任务的交付记录区域：

1. **任务开始 checkpoint**：任务状态从 `TODO` 变为 `DOING` 时，记录当前 working tree 状态（`git diff --stat` 或文件列表）。
2. **子步骤 checkpoint**：每完成一个可验证的子步骤（如：一个函数实现 + 对应测试通过），追加 checkpoint。
...
当任务执行遇到失败（测试失败、编译错误、外部依赖不可用）时，按以下优先级尝试恢复：

| 优先级 | Action | 适用场景 | 最大尝试次数 |
| --- | --- | --- | --- |
| 1 | **Retry** | 瞬时错误（网络超时、flaky test） | 1 次 |
| 2 | **Fix-forward** | 可定位的代码错误（类型错误、逻辑 bug） | 2 次 |
| 3 | **Rollback-to-checkpoint** | Fix-forward 失败且有 passing checkpoint | 1 次 |
| 4 | **Escalate** | 无 checkpoint 或回退后仍失败 | — |

Recovery action 规则：

- **Retry**：重新执行失败的命令或测试，不修改代码。如果重试后仍失败，进入 Fix-forward。
- **Fix-forward**：分析错误根因，修改代码修复。每次 fix-forward 后必须重新运行验证。最多尝试 2 次不同的修复路径。
- **Rollback-to-checkpoint**：回退到最近的 `tests_passing: true` checkpoint 对应的文件状态，然后尝试不同的实现路径。
- **Escalate**：标记任务为 `BLOCKED`，记录完整错误证据（错误信息、已尝试的修复路径、失败原因），停止并报告。

### Recovery 记录

每次 recovery 必须在任务交付记录中写入：
...
recovery_log:
  - timestamp: "ISO 8601 + timezone"
    ...
    checkpoint_used: "cp_01"  # 仅 rollback 时填写
    attempts: 1
```

### Reflection（自我批评）

Recovery 完成后（无论成功或 escalate），必须在任务交付记录中写一段 reflection：

```markdown
#### Reflection

- **根因分析**：为什么失败？是 design 缺陷、实现路径错误、还是外部依赖问题？
- **预防措施**：下次如何避免同类问题？
- **上游影响**：是否需要更新 `design.md` 或 `tasks.md`？如果是，记录具体需要更新的内容。
```

### 与其他 contract 的关系

- Recovery 过程中仍遵守 Gate policy：不得跳过当前任务执行下一个。
- Recovery 过程中仍遵守 Safety policy：不得擅自删除文件或回滚用户未提交改动。
- Rollback-to-checkpoint 只回退 AI 在当前任务中的改动，不影响用户的其他工作。
- Escalate 后的 `BLOCKED` 状态遵守 Task status contract：需要外部条件解除后才能继续。
- 每次 recovery 必须追加 `error_recovery` 事件到 `metrics.json`（参见 section 16）。
